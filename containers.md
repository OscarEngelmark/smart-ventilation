...
| `room-model` | Learns how the room's CO2 responds to people and to the damper: the three rates of the room model | Once per room day, stored CO2 readings, head counts and damper commands, from `GET /co2`, `GET /occupancy` and `GET /commands` | The three rates, as a retained MQTT message; topic not yet chosen | `storage-service`, `mosquitto` | Fit written (`internal/roommodel`); service planned (working plan row 6) |
